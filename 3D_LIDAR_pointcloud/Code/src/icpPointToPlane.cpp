...
double IcpPointToPlane::fitStep (double *T,const long T_num,Matrix &R,Matrix &t,const std::vector<long> &active) {
...
std::vector<long> IcpPointToPlane::getInliers (double *T,const long T_num,const Matrix &R,const Matrix &t,const double indist) {
...
double* IcpPointToPlane::computeNormals (const long num_neighbors,const double flatness) {
  double *M_normal = (double*)malloc(M_tree->N*dim*sizeof(double));
  kdtree::KDTreeResultVector neighbors;
  for (long i=0; i<M_tree->N; i++) {
