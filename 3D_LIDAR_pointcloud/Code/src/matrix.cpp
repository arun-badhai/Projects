...
Matrix::Matrix (const long m_,const long n_) {
  allocateMemory(m_,n_);
}

Matrix::Matrix (const long m_,const long n_,const FLOAT* val_) {
  allocateMemory(m_,n_);
  long k=0;
  for (long i=0; i<m_; i++)
    for (long j=0; j<n_; j++)
...
void Matrix::getData(FLOAT* val_,long i1,long j1,long i2,long j2) {
  if (i2==-1) i2 = m-1;
  if (j2==-1) j2 = n-1;
  long k=0;
...
Matrix Matrix::getMat(long i1,long j1,long i2,long j2) {
  if (i2==-1) i2 = m-1;
  if (j2==-1) j2 = n-1;
...
void Matrix::setMat(const Matrix &M,const long i1,const long j1) {
...
void Matrix::setVal(FLOAT s,long i1,long j1,long i2,long j2) {
  if (i2==-1) i2 = m-1;
  if (j2==-1) j2 = n-1;
  if (i2<i1 || j2<j1) {
...
void Matrix::setDiag(FLOAT s,long i1,long i2) {
...
Matrix Matrix::eye (const long m) {
  Matrix M(m,m);
  for (long i=0; i<m; i++)
    M.val[i][i] = 1;
  return M;
...
Matrix Matrix::ones (const long m,const long n) {
  Matrix M(m,n);
  for (long i=0; i<m; i++)
    for (long j=0; j<n; j++)
      M.val[i][j] = 1;
...
Matrix Matrix::reshape(const Matrix &M,long m_,long n_) {
  if (M.m*M.n != m_*n_) {
...
FLOAT Matrix::det () {
  ...
  Matrix A(*this);
  long *idx = (long*)malloc(m*sizeof(long));
  FLOAT d;
  A.lu(idx,d);
  for( long i=0; i<m; i++)
    d *= A.val[i][i];
...
bool Matrix::solve (const Matrix &M, FLOAT eps) {
  ...
  long* indxc = new long[m];
  long* indxr = new long[m];
  long* ipiv  = new long[m];
...
bool Matrix::lu(long *idx, FLOAT &d, FLOAT eps) {
...
void Matrix::allocateMemory (const long m_,const long n_) {
  m = abs(m_); n = abs(n_);
  if (m==0 || n==0) {
    val = 0;
    return;
